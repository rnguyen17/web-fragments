...
export function getWebMiddleware(
  ...
  return async (request: Request, next: () => Promise<Response>): Promise<Response> => {
    ...
    const fragmentResponsePromise =
      ...
      matchedFragment.piercing || requestSecFetchDest !== 'document' ? fetchFragment(request, matchedFragment, additionalHeaders, mode) : undefined; //Promise.reject('Unexpected fragment request');
        ...
        response: new Response(
          mode === 'development'
            ? `<p>Failed to fetch fragment!<br>
										...
										Response: HTTP ${
                      fragmentResponseOrError instanceof Response
                        ? `${fragmentResponseOrError.status} ${fragmentResponseOrError.statusText}<br>${await fragmentResponseOrError.text()}`
  ...
  async function fetchFragment(
    ...
    fragmentReq.headers.set('x-forwarded-proto', originalRequest.headers.get('x-forwarded-proto') || requestUrl.protocol.slice(0, -1));
    ...
    if (isNativeHtmlRewriter) {
      ...
      return new HTMLRewriter()
        ...
        .on('body', {
          element(element) {
            element.append(
              `<web-fragment-host class="${prePiercingClassNames.join(
                ' '
              )}" fragment-id="${fragmentId}" data-piercing="true"><template shadowrootmode="open">${fragmentContent}</template></web-fragment-host>`,
              { html: true }
            );
          },
        })
